...
void ata_write(int drive, int lba, uint8_t* buffer, uint32_t sectors) {
    log_group_begin("ATA Write");
    log_info("Drive  ", "%x", drive);
    log_info("LBA    ", "%d", lba);
    log_info("AbsAddr", "dec (%d), hex(0x%x)", lba * 512, lba * 512);
    log_info("Sectors", "%d", sectors);
    log_info("buf_add", "%x", buffer);
    log_group_end("ATA Write");
    ...
    for (uint32_t i = 0; i < sectors * 256; i++) {
        while (!(io_inb(ATA_STATUS) & ATA_STATUS_DRQ)) {
            // log_info("Waiting for DRQ", "");
            if (io_inb(ATA_STATUS) & ATA_STATUS_ERR) { // Handle error
                uint8_t err = io_inb(ATA_ERROR);
                log_crit("ATA Write", "Error writing sector %d: ERR=%x\n", lba, err);
                return;                                 // Exit on error
            }
        }
        uint16_t data = (uint16_t)(buffer[i * 2] | (buffer[i * 2 + 1] << 8));  
        io_outw(0x1F0, data);
        // log_info("ATA Write", "wrote %X %X", data & 0xff, (data >> 8) & 0xff);
    }
}
