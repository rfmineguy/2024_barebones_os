...
void multiboot_debug(struct multiboot_header* header, struct multiboot_info* info) {
  log_info("MultibootDebug",  "header_p: %x"     "\n"   , header);
  log_info("MultibootDebug",  "info_p: %x"       "\n"   , info);
  log_info("MultibootHeader", "magic: %x"        "\n"   , header->magic);
  log_info("MultibootHeader", "flags: %b"        "\n"   , header->flags);
  log_info("MultibootHeader", "checksum: %x"     "\n"   , header->checksum);
  log_info("MultibootHeader", "header_addr: %x"  "\n"   , header->header_addr);
  log_info("MultibootHeader", "load_addr: %x"    "\n"   , header->load_addr);
  log_info("MultibootHeader", "bss_end_addr: %x" "\n"   , header->bss_end_addr);
  log_info("MultibootHeader", "entry_addr: %x"   "\n"   , header->entry_addr);
  log_info("MultibootHeader", "mode_type: %d"    "\n"   , header->mode_type);
  log_info("MultibootHeader", "width: %d"        "\n"   , header->width);
  log_info("MultibootHeader", "height: %d"       "\n"   , header->height);
  log_info("MultibootHeader", "depth: %d"        "\n"   , header->depth);
  ...
  log_info("Multiboot", "drives_addr: %x"        "\n"   , info->drives_addr);
  log_info("Multiboot", "config_table: %d"       "\n"   , info->config_table);
  log_info("Multiboot", "boot_loader_name: %s"   "\n"   , info->boot_loader_name);
