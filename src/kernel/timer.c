...
#define DATA_1 0x41         // Channel 1 data port (read/write)
...
void timer_init() {
    ...
    io_outb(MODE_CMD, 0x36); io_wait();

    // set the frequency
    io_outb(DATA_0, (uint8_t)(divisor & 0xff)); io_wait();
    io_outb(DATA_0, (uint8_t)((divisor >> 8) & 0xff)); io_wait();
