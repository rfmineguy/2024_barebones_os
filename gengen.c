...
int main() {
  ...
  replacement_add(&event_queue_repl, "^T", "event");
  replacement_add(&event_queue_repl, "SUPPORT_1", "event.h");
  ...
  replacement_add(&memory_linked_list_repl, "^T", "memory_node");
  replacement_add(&memory_linked_list_repl, "SUPPORT_1", "memory_node.h");
  ...
  generator_run(gen_settings(.search_paths=paths("src/tpl"), .outdir="src/kernel", .verbose = false), queue, event_queue_repl);
  generator_run(gen_settings(.search_paths=paths("src/tpl"), .outdir="src/kernel", .verbose = false), linkedlist, memory_linked_list_repl);
}
